...
if __name__ == "__main__":
    ...
    rampoffvals = [0.0, 0.0, -4900.0, -4900.0, -4900.0, -4900.0, -4900.0, -4900.0]
    ...
    line_init = (
        Exponential1D(
            x_0=-2.0,
            amplitude=-500.,
            bounds={"amplitude": [-100000.0, -100.0], "x_0": [-4.0, 0.0]},
        )
        + Linear1D()
    )
    ...
    for z, cfile in enumerate(filenames):
        ...
        for k in range(nints):
            gnum, ydata = get_ramp(
                hdu[0], pix_x, pix_y, k, rampoffval=rampoffvals[z + 1]
            )
            ggnum, gdata, aveDN, diffDN = get_good_ramp(gnum, ydata)
    ...
    out_basename = f"plot_miri_ramp_ramp_{pix_x}_{pix_y}_nreg{args.nrej+1}"
    if args.primeonly:
